...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    //image upload
    $image_path = $restaurant['image_path']; // default

    if ($remove_image) {
        // Remove existing image
        if (!empty($restaurant['image_path'])) {
            $image_file = '../uploads/images/' . $restaurant['image_path'];
        ...
        if (in_array($fileExt, $allowed)) {
            if ($fileSize < 5000000) { // 5MB max
                ...
                if (!empty($restaurant['image_path'])) {
                    $old_file = '../uploads/images/' . $restaurant['image_path'];
    ...
    if (empty($name) || empty($description)) {
        ...
        try {
            $stmt = $pdo->prepare("
                UPDATE restaurants
                SET name = ?, description = ?, address = ?, phone = ?, category_id = ?, image_path = ?, updated_at = NOW()
                WHERE id = ?
            ");
            $stmt->execute([$name, $description, $address, $phone, $category_id, $image_path, $id]);
...
<head>
    ...
    <title>Edit Restaurant - Restaurant CMS</title>
                ...
                <form method="post" enctype="multipart/form-data">
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select" id="category_id" name="category_id">
                                ...
                                <?php foreach ($categories as $category): ?>
                                    <option value="<?php echo $category['id']; ?>" <?php echo $restaurant['category_id'] == $category['id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($category['name']); ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <?php if (!empty($restaurant['image_path'])): ?>
                            <div class="mb-2">
                                <img src="../uploads/images/<?php echo htmlspecialchars($restaurant['image_path']); ?>" class="current-image" alt="Current image">
